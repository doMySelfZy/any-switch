...
use crate::domain::{
    provider_id_for_site, ApplyStatus, PrimeApplyOptions, SiteProtocol, SiteRow, TargetBinding,
    TargetKind, TouchedKeys,
};
use crate::error::{AppError, AppResult};
use crate::paths::{resolve_prime_agent_dir, set_secret_permissions};
use chrono::Utc;
...
pub fn models_path(override_path: Option<&str>) -> AppResult<PathBuf> {
    Ok(resolve_prime_agent_dir(override_path)?.join("models.json"))
}

pub fn auth_path(override_path: Option<&str>) -> AppResult<PathBuf> {
    Ok(resolve_prime_agent_dir(override_path)?.join("auth.json"))
}

pub fn settings_path(override_path: Option<&str>) -> AppResult<PathBuf> {
    Ok(resolve_prime_agent_dir(override_path)?.join("settings.json"))
}
...
fn parse_models(text: &str) -> AppResult<CstRootNode> {
    if contains_block_comment(text) {
        return Err(AppError::new(
            "invalid_config",
            "Prime models.json contains a block comment unsupported by Prime",
        ));
    }
    let root = CstRootNode::parse(text, &parse_options())
        .map_err(|e| AppError::new("invalid_config", format!("invalid Prime models.json: {e}")))?;
    if root.object_value().is_none() {
        return Err(AppError::new(
            "invalid_config",
            "Prime models.json root must be an object",
        ));
    }
    Ok(root)
}
...
fn read_json_object(path: &Path, label: &str) -> AppResult<Value> {
    ...
    let value: Value = serde_json::from_str(&fs::read_to_string(path)?)
        .map_err(|e| AppError::new("invalid_config", format!("invalid Prime {label}: {e}")))?;
    if !value.is_object() {
        return Err(AppError::new(
            "invalid_config",
            format!("Prime {label} root must be an object"),
        ));
    }
    Ok(value)
}
...
fn upsert_managed_provider(
    ...
    let object = root
        .object_value()
        .ok_or_else(|| AppError::new("invalid_config", "Prime models.json root must be object"))?;
    let providers = object.object_value_or_create("providers").ok_or_else(|| {
        AppError::new(
            "invalid_config",
            "Prime models.json providers must be object",
        )
    })?;
...
fn restore_baseline(settings: &mut Value, binding: &TargetBinding) -> AppResult<()> {
    let object = settings.as_object_mut().ok_or_else(|| {
        AppError::new("invalid_config", "Prime settings.json root must be object")
    })?;
...
impl PrimeFileLock {
    fn acquire(file: &Path) -> AppResult<Self> {
        let file_name = file
            ...
            .ok_or_else(|| AppError::new("internal", "invalid Prime config path"))?;
        ...
        Err(AppError::new(
            "lock_busy",
            format!("Prime is using {}", file.display()),
        ))
    }
}

impl Drop for PrimeFileLock {
    fn drop(&mut self) {
...
struct PrimeLocks {
    _auth: PrimeFileLock,
    _settings: PrimeFileLock,
}

impl PrimeLocks {
    fn acquire(auth: &Path, settings: &Path) -> AppResult<Self> {
        Ok(Self {
            _auth: PrimeFileLock::acquire(auth)?,
            _settings: PrimeFileLock::acquire(settings)?,
        })
    }
}
...
fn write_three_files(
    ...
    if let Err(error) = result {
        rollback_files(originals).map_err(|rollback| {
            AppError::new(
                "atomic_write_failed",
                format!("{error}; Prime rollback failed: {rollback}"),
            )
        })?;
        return Err(error);
    }
...
fn fail_test_write_at(step: usize) -> AppResult<()> {
    TEST_FAIL_WRITE_STEP.with(|target| {
        if target.get() == step {
            ...
            Err(AppError::new(
                "atomic_write_failed",
                format!("injected Prime write failure at step {step}"),
            ))
        } else {
            Ok(())
        }
...
pub fn apply(
    ...
    backup_root: &Path,
) -> AppResult<PrimeApplyOutcome> {
    let agent_dir = resolve_prime_agent_dir(override_path)?;
    ...
    let _locks = PrimeLocks::acquire(&auth_path, &settings_path)?;

    // Prime may have written auth/settings before the lock was acquired.
    ...
    crate::domain::validate_write(&thinking, TargetKind::Prime, site.protocol, model_id)?;
    let write_opts = PrimeApplyOptions {
    ...
    let auth_object = auth
        .as_object_mut()
        .ok_or_else(|| AppError::new("invalid_config", "Prime auth.json root must be object"))?;
    ...
    let verified = (|| {
        ...
        let provider = provider(&models, &provider_id)
            .ok_or_else(|| AppError::new("invalid_config", "Prime provider missing after write"))?;
    ...
    Ok(PrimeApplyOutcome {
        binding,
        touched,
        backup_paths,
        live_summary: summary,
        message:
            "Prime models.json, auth.json and settings.json updated. Start a new Prime session."
                .into(),
        provider_id,
    })
}
...
pub fn detect_status(
    ...
    let Some(binding) = binding else {
        return Ok(if orphan {
            (ApplyStatus::Orphan, Some("untracked Prime provider".into()))
        } else {
    ...
    let provider_id = binding
        .provider_id
        .as_deref()
        .ok_or_else(|| AppError::new("invalid_config", "Prime binding provider id missing"))?;
    let Some(provider) = provider(&models, provider_id) else {
        return Ok((ApplyStatus::Stale, Some("Prime provider missing".into())));
    };
            ...
            != Some(current_base_url.as_str())
    {
        return Ok((
            ApplyStatus::Stale,
            Some("Prime site protocol or Base URL changed".into()),
        ));
    }
    for (field, expected) in [("baseUrl", "base_url"), ("api", "api")] {
        ...
        {
            return Ok((ApplyStatus::Stale, Some(format!("Prime {field} changed"))));
        }
    }
    ...
    if !model_exists {
        return Ok((
            ApplyStatus::Stale,
            Some("Prime selected model missing".into()),
        ));
    }
    ...
    if live_key.as_deref().map(key_fingerprint).as_deref() != Some(&expected_fingerprint) {
        return Ok((ApplyStatus::Stale, Some("Prime API key changed".into())));
    }
...
pub fn rewrite_base_url(
    ...
    let provider = object
        ...
        .ok_or_else(|| AppError::new("invalid_config", "managed Prime provider missing"))?;
    ...
    Ok(RewriteOutcome {
        ...
        expected_fields: expected,
        message: "Prime provider route updated.".into(),
    })
}
...
mod tests {
    ...
    #[test]
    fn rejects_jsonc_features_that_prime_does_not_support() {
    ...
    fn rolls_back_all_files_when_the_last_write_fails() {
        ...
        .unwrap_err();
        assert!(error.to_string().contains("injected Prime write failure"));
        assert_eq!(
    ...
    #[test]
    fn writes_reasoning_on_openai_compatible() {
        let dir = tempdir().unwrap();
        let mut thinking = ThinkingWrite::default();
        thinking.default_level = Some("high".into());
    ...
    #[test]
    fn anthropic_rejects_extended_default() {
        let dir = tempdir().unwrap();
        let mut thinking = ThinkingWrite::default();
