...
- 目标：Claude Code（`~/.claude/settings.json`）、Codex（`~/.codex/` + 环境变量注入）与 Pi（`~/.pi/agent/{models,auth,settings}.json`）
...
- API key 在数据库中加密存储；**应用（Apply）** 之后，它们可能以明文出现在 `~/.claude` / `~/.codex` / `~/.pi/agent/auth.json` / `codex.env` / shell rc 中 —— 须在 UI 文案中说明这一点。
...
- 当前目标为 **Claude Code**、**Codex** 与 **Pi**；图标来自 `@lobehub/icons`（`ClaudeCode`、`Codex`、`Pi`）
