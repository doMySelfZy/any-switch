...
AnySwitch 使用 Prime 官方配置接口，不安装 extension：
...
模型 ID、显示名称与图像输入会写入目录。思考能力不会按模型名称猜测：需要在应用中心逐模型开启 `reasoning`，并可选写入 `defaultThinkingLevel`。OpenAI 兼容协议可配置 `xhigh` / `max` 映射；Anthropic 协议本次只提供通用五档。上下文窗口、最大输出等其余字段仍使用 Prime 官方默认值。

## 保留与单活

Prime 中同时只保留一个 AnySwitch 管理的 Provider。切换站点只替换 `xiaobai_` 命名空间；其他自定义 Provider、OAuth 登录、注释、尾逗号和未知设置都会保留。
