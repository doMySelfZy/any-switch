...
import { Alert, App, Select, Skeleton, Space, Switch, theme } from "antd";
import { useTranslation } from "react-i18next";
import { SettingsGroup } from "@/components/settings/SettingsGroup";
import { useApplyStore, useSiteStore } from "@/stores";
import { isAppError } from "@/lib/invoke";
import type { ZCodeApiType } from "@/types/domain";
...
const API_TYPE_OPTIONS: { value: ZCodeApiType; labelKey: string }[] = [
  { value: "anthropic-messages", labelKey: "apply.zcodeApiTypeAnthropic" },
  { value: "openai-responses", labelKey: "apply.zcodeApiTypeResponses" },
  { value: "openai-chat-completions", labelKey: "apply.zcodeApiTypeChatCompletions" },
];
...
export const ZCodeApplyPanel = memo(function ZCodeApplyPanel() {
  ...
  const [apiType, setApiType] = useState<ZCodeApiType>(() => inferZCodeApiType(undefined));
  ...
  useEffect(() => {
    ...
    if (
      ...
      return;
    const defaults = hydrateZCodeForm(site, status);
  ...
  /** 站点里实际存着的协议：null 表示由站点协议推断。 */
  const storedApiType = site ? (site.zcodeApiType ?? inferZCodeApiType(site.protocol)) : undefined;

  /** 把协议选择写回站点；失败时抛出，由调用方决定怎么提示。 */
  const saveApiType = async (next: ZCodeApiType) => {
    if (!site || storedApiType === next) return;
    await updateSite(site.id, { zcodeApiType: next });
  };

  const handleApiTypeChange = (next: ZCodeApiType) => {
    setApiType(next);
    void saveApiType(next).catch((error) => {
      message.error(isAppError(error) ? error.message : String(error));
    });
  };
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // 协议必须先落库：后端是从站点行读 zcode_api_type 的。
      await saveApiType(apiType);
      ...
      showApplyOutcome(
        modal,
        t,
        result.results.find((item) => item.target === "zcode"),
      );
    } catch (error) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto p-6 pb-4">
        ...
        {site && (
          <SettingsGroup title={t("apply.groupZCodeProtocol")}>
            <div style={rowStyle}>
              <div className="mb-1 text-sm opacity-70">{t("apply.zcodeApiType")}</div>
              <Select<ZCodeApiType>
                className="w-full"
                value={apiType}
              ...
              <div className="mt-1 text-xs opacity-50">
                {t("apply.zcodeApiTypeHint", { protocol: site.protocol })}
              </div>
            </div>
          </SettingsGroup>
        )}
        ...
        {site && (
          <SettingsGroup title={t("apply.groupZCodeModels")}>
            <div style={rowStyle} className="flex items-center justify-between gap-4">
              <div>
                <div>{t("apply.zcodeWriteAllModels")}</div>
                <div className="text-xs opacity-50">{t("apply.zcodeWriteAllModelsHint")}</div>
              </div>
